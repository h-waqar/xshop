...
jQuery(function ($) {
    ...
    class XshopTopup {
        ...
        bindEvents() {
            ...
            this.$modal.on('click', '.xshop-confirm', (e) => {
                const $btn = $(e.currentTarget);
                ...
                this.addToCart().finally(() => {
                    // if page redirects, this won't matter, but in case of error we restore
        ...
        addToCart() {
            ...
            let sku = '';
            if (validateResult?.message?.sku) {
                sku = validateResult.message.sku.sku || validateResult.message.sku.code || '';
            }

            // return the $.post promise so caller can attach .finally()
            return $.post(xshopValidateConfig.ajax_url, {
                action: 'xshop_add_to_cart',
                nonce: xshopValidateConfig.nonce,
                product_id: productId,
                variation_id: variationId,
                ...
                if (res && res.success) {
                    const redirect = (res.data && res.data.redirect)
                        ? res.data.redirect
                        : xshopValidateConfig.checkout_url;
                    window.location.href = redirect;
                } else {
